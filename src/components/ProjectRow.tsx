...
export default function ProjectRow({ title, projects }: Props) {
  if (projects.length === 0) return null

  return (
    <div>
      <h3 className="font-display font-semibold text-lg sm:text-xl text-text mb-4">{title}</h3>
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-4 px-4 sm:-mx-6 sm:px-6 snap-x snap-mandatory [scrollbar-width:thin]">
        {projects.map((project) => (
          <Link
            key={project.id}
            to={`/projects/${project.slug}`}
            className="group shrink-0 w-60 sm:w-72 snap-start rounded-xl overflow-hidden bg-surface border border-white/[0.07] hover:border-cinema/40 hover:-translate-y-1 transition-all duration-300"
          >
